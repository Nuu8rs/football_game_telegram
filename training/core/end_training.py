...
from training.utils.get_count_stats_by_score import calculate_stat
...
class EndTraining:
    ...
    async def end_training(self) -> None:
        ...
        if not self.count_stat:
            return await self.__send_message(
                text = TEXT_TRAINING.FAILED_END_TRAINING
            )
        return await self.__send_message(
            text = TEXT_TRAINING.TRAINING_STATS_SELECTION,
